...
describe('Testing all functions in run file.', () => {
   ...
   test('downloadKubectl() - custom mirror: 200 streams body to temp file and caches', async () => {
      ...
      // Assert the body actually streamed to disk (not an empty write).
      const [writePath, writeContent] = vi.mocked(fs.writeFileSync).mock
         .calls[0]
      expect(String(writePath)).toMatch(/[/\\]kubectl-[0-9a-f-]+$/)
      expect((writeContent as Buffer).toString()).toBe('kubectl-bytes')
   })
   ...
   test('downloadKubectl() - cache key is scoped per non-default baseURL', async () => {
      ...
      await run.downloadKubectl('v1.15.0', 'https://mirror.example.com')
      ...
      await run.downloadKubectl('v1.15.0', 'https://other-mirror.example.com')
   ...
   test('downloadKubectl() - custom mirror: temp file is unlinked after caching', async () => {
      ...
      // The same temp path written by secureDownload must be unlinked afterwards.
      const writtenPath = String(vi.mocked(fs.writeFileSync).mock.calls[0][0])
      expect(writtenPath).toMatch(/[/\\]kubectl-[0-9a-f-]+$/)
